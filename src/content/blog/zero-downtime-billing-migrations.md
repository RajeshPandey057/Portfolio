...
3. **Suppress, then verify, then release.** After creating the subscription in Stripe, the WooCommerce renewal does not get deleted. It gets suppressed (renewals disabled) and retained as evidence. Deleting the fallback before validating the replacement is how you turn a bug into an outage.
